...
{	int j, h = now._nr_pr;
	...
	case 2:	/* :init: */
		...
		((P2 *)pptr(h))->_6_2_i = 0;
#ifdef VAR_RANGES
		logval(":init::i", ((P2 *)pptr(h))->_6_2_i);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Centralita */
		...
		((P1 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("Centralita:estado", ((P1 *)pptr(h))->estado);
		logval("Centralita:i", ((P1 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Vecino */
		...
		((P0 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("Vecino:estado", ((P0 *)pptr(h))->estado);
		logval("Vecino:id", ((P0 *)pptr(h))->id);
		logval("Vecino:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.ctr = addqueue(calling_pid, 1, 1);
		now.ctr2v[0] = addqueue(calling_pid, 2, 1);
		now.ctr2v[1] = addqueue(calling_pid, 3, 1);
		now.ctr2v[2] = addqueue(calling_pid, 4, 1);
		now.ctr2v[3] = addqueue(calling_pid, 5, 1);
		now.ctr2v[4] = addqueue(calling_pid, 6, 1);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ctr) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ctr) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ctr) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ctr) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ctr2v[0]) )
			 ||     q_full((int) ( now.ctr2v[1]) )
			 ||     q_full((int) ( now.ctr2v[2]) )
			 ||     q_full((int) ( now.ctr2v[3]) )
			 ||     q_full((int) ( now.ctr2v[4]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ctr2v[0]) )
			 ||    !q_full((int) ( now.ctr2v[1]) )
			 ||    !q_full((int) ( now.ctr2v[2]) )
			 ||    !q_full((int) ( now.ctr2v[3]) )
			 ||    !q_full((int) ( now.ctr2v[4]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ctr2v[0]) )
			 ||     !q_len((int) ( now.ctr2v[1]) )
			 ||     !q_len((int) ( now.ctr2v[2]) )
			 ||     !q_len((int) ( now.ctr2v[3]) )
			 ||     !q_len((int) ( now.ctr2v[4]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ctr2v[0]) )
			 ||      q_len((int) ( now.ctr2v[1]) )
			 ||      q_len((int) ( now.ctr2v[2]) )
			 ||      q_len((int) ( now.ctr2v[3]) )
			 ||      q_len((int) ( now.ctr2v[4]) )))
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  sonando:	5\n");
	printf("	mtype  descolgado:	4\n");
	printf("	mtype  colgado:	3\n");
	printf("	mtype  conectado:	2\n");
	printf("	mtype  responde:	1\n");
	printf("	chan ctr (=%d):	len %d:\t", now.ctr, q_len(now.ctr));
	c_chandump(now.ctr);
	printf("	chan ctr2v[0] (=%d):	len %d:\t", now.ctr2v[0], q_len(now.ctr2v[0]));
	c_chandump(now.ctr2v[0]);
	printf("	chan ctr2v[1] (=%d):	len %d:\t", now.ctr2v[1], q_len(now.ctr2v[1]));
	c_chandump(now.ctr2v[1]);
	printf("	chan ctr2v[2] (=%d):	len %d:\t", now.ctr2v[2], q_len(now.ctr2v[2]));
	c_chandump(now.ctr2v[2]);
	printf("	chan ctr2v[3] (=%d):	len %d:\t", now.ctr2v[3], q_len(now.ctr2v[3]));
	c_chandump(now.ctr2v[3]);
	printf("	chan ctr2v[4] (=%d):	len %d:\t", now.ctr2v[4], q_len(now.ctr2v[4]));
	c_chandump(now.ctr2v[4]);
}
void
...
{	/* int i; */
	...
	printf("	short  i:	%d\n", ((P2 *)pptr(pid))->_6_2_i);
		break;
	case 1:
		printf("local vars proc %d (Centralita):\n", pid);
	printf("	mtype  estado:	%d\n", ((P1 *)pptr(pid))->estado);
	printf("	short  i:	%d\n", ((P1 *)pptr(pid))->i);
		break;
	case 0:
		printf("local vars proc %d (Vecino):\n", pid);
	printf("	short  id:	%d\n", ((P0 *)pptr(pid))->id);
	printf("	mtype  estado:	%d\n", ((P0 *)pptr(pid))->estado);
	printf("	short  i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
...
{
	switch (x) {
	case 1: Printf("responde"); break;
	case 2: Printf("conectado"); break;
	case 3: Printf("colgado"); break;
	case 4: Printf("descolgado"); break;
	case 5: Printf("sonando"); break;
	default: Printf("%d", x);
	}
}
void
